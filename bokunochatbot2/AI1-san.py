...
qdrant_apikey= st.secrets["QDRANT_API_KEY"]
...
def main():
    ...
    if process:
        if not openai_api_key:
            st.info("Please  add your OPENAI API key to continue this process")
            st.stop()
    ...
    if st.session_state.processComplete==True:
        user_question= st.chat_input("Ask any Question about your files.")
        if user_question:
...
def get_vectorstore(tchunks,cname):
    try:
